...
def show_critical_path_page():
    ...
    # =========================================================================
    # TAB 6: DOCUMENT SCANNER AGENT
    # =========================================================================
    with tab6:
        st.subheader("📄 Document Scanner Agent")
        st.info("Scan emails and meeting notes for milestone updates using AI")
        
        # Scan history display
        scan_history = cp_data.document_scan_history
        if scan_history:
            last_scan = scan_history.get('last_scan_date', 'Never')
            st.caption(f"Last scan: {last_scan}")
        
        # Folder input
        folder_path = st.text_input(
            "📁 Document Folder Path",
            placeholder="/Users/you/Documents/ProjectEmails",
            help="Path to folder containing project documents"
        )
        
        # File type selection
        file_types = st.multiselect(
            "File Types to Scan",
            ["PDF", "DOCX", "TXT", "EML", "MSG"],
            default=["PDF", "TXT"],
        ...
        with col1:
            if st.button("🚀 Scan Documents", type="primary"):
                if not folder_path:
                    ...
                    with st.spinner("Scanning documents with AI..."):
                        try:
                            results = scan_documents_for_updates(
                                folder_path,
                                file_types,
                                selected_site_id,
                                cp_data
        ...
        if 'scan_results' in st.session_state:
            ...
            st.success(f"✅ Scanned {results['files_scanned']} files")
            ...
            if results['updates']:
                st.metric("Updates Found", len(results['updates']))
                
                st.subheader("📋 Detected Updates")
                
                for idx, update in enumerate(results['updates']):
                    with st.expander(
                        f"{update.get('milestone_name', update['milestone_id'])} - {update['update_type']}"
                    ):
                        col1, col2 = st.columns([2, 1])
                        
                        with col1:
                            st.write(f"**Source**: {update['source_file']}")
                            st.write(f"**Confidence**: {update.get('confidence', 0)*100:.0f}%")
                            st.code(update.get('excerpt', 'No excerpt'), language=None)
                            st.write(f"**Update**: `{update.get('old_value')}` → `{update.get('new_value')}`")
                        
                        with col2:
                            if not update.get('applied'):
                                if st.button("✅ Apply", key=f"apply_doc_{idx}"):
                                    if apply_document_update(cp_data, update):
                                        # Recalculate schedule
                                        for ms in cp_data.milestones.values():
                                            ms.target_start = ms.target_end = None
                                        cp_data = engine.calculate_schedule(cp_data)
                                ...
                                if st.button("❌ Reject", key=f"reject_doc_{idx}"):
                                    results['updates'][idx]['rejected'] = True
                                    st.info("Rejected")
                            else:
                                st.success("✅ Applied")
            else:
                st.info("No updates found in scanned documents")
    
    # =========================================================================
    # TAB 7: WEB INTELLIGENCE AGENT
    # =========================================================================
    with tab7:
        st.subheader("🌐 Web Intelligence Agent")
        st.info("AI-powered research on equipment lead times and industry timelines")
        
        # Intelligence database info
        intel_db = cp_data.intelligence_database if cp_data.intelligence_database else {}
        last_updated = intel_db.get('last_updated', 'Never')
        st.caption(f"Database last updated: {last_updated}")
        
        # Research controls
        st.write("### Research Controls")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("🔌 Research Equipment", type="primary"):
                with st.spinner("Researching equipment lead times..."):
                    try:
                        ...
                        from .streamlit_app import save_database
                        save_database(db)
                        st.success("Equipment research complete!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Research error: {str(e)}")
        
        with col2:
            if st.button("⚡ Research ISO Timelines"):
                with st.spinner("Researching ISO timelines..."):
                    try:
                        ...
                        from .streamlit_app import save_database
                        save_database(db)
                        st.success("ISO research complete!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Research error: {str(e)}")
        
        with col3:
            if st.button("🔍 Full Research Scan"):
                with st.spinner("Comprehensive research (this may take a minute)..."):
                    try:
                        research_all_intelligence(cp_data)
                        site = save_critical_path_to_site(site, cp_data)
                        sites[selected_site_id] = site
                        from .streamlit_app import save_database
                        save_database(db)
                        st.success("Full research complete!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Research error: {str(e)}")
        
        # Display Equipment Lead Times
        if intel_db.get('equipment_lead_times'):
            st.write("---")
            st.write("### ⚡ Equipment Lead Times Database")
            
            for equip_id, data in intel_db['equipment_lead_times'].items():
                with st.expander(f"🔧 {equip_id.replace('_', ' ').title()}"):
                    col1, col2, col3, col4 = st.columns(4)
                    
                    with col1:
                        st.metric("Typical", f"{data.get('typical_weeks', 0)} wks")
                    
                    with col2:
                        current = data.get('current_weeks', 0)
                        typical = data.get('typical_weeks', 0)
                        delta = current - typical
                        st.metric(
                            "Current (2025)",
                            f"{current} wks",
                            delta=f"+{delta} wks" if delta > 0 else f"{delta} wks"
                        )
                    
                    with col3:
                        trend = data.get('trend', 'stable')
                        trend_emoji = "📈" if trend == "increasing" else "📊" if trend == "stable" else "📉"
                    ...
                    # Apply button
                    if st.button(f"✅ Apply to Schedule", key=f"apply_equip_{equip_id}"):
                        if apply_intelligence_to_schedule(cp_data, equip_id, data):
                            # Recalculate
                            for ms in cp_data.milestones.values():
                                ms.target_start = ms.target_end = None
                            cp_data = engine.calculate_schedule(cp_data)
                            ...
                            st.success(f"✅ Applied {data['current_weeks']} weeks to schedule!")
                            st.rerun()
                        else:
                            st.error("Could not apply - milestone not found")
        
        # Display ISO Timelines
        if intel_db.get('iso_timelines'):
            st.write("---")
            st.write("### 🔌 ISO Interconnection Timelines")
            
            # Create table
            iso_table_data = []
            for iso_name, timelines in intel_db['iso_timelines'].items():
                iso_table_data.append({
                    'ISO': iso_name,
                    'Screening': f"{timelines.get('screening_weeks', 'N/A')} wks",
                    'SIS': f"{timelines.get('sis_weeks', 'N/A')} wks",
                    'FS': f"{timelines.get('fs_weeks', 'N/A')} wks",
                    'Total': f"{timelines.get('total_process_weeks', 'N/A')} wks",
                    'Trend': timelines.get('trend', 'stable').title()
                })
            
            st.table(pd.DataFrame(iso_table_data))
            
            # Apply specific ISO timelines
            st.write("**Apply ISO Timelines:**")
            iso_list = list(intel_db['iso_timelines'].keys())
            selected_iso = st.selectbox("Select ISO to apply", iso_list)
            
            if st.button(f"✅ Apply {selected_iso} Timelines"):
                timeline_data = intel_db['iso_timelines'][selected_iso]
                if apply_iso_intelligence_to_schedule(cp_data, selected_iso, timeline_data):
                    # Recalculate
                    for ms in cp_data.milestones.values():
                        ms.target_start = ms.target_end = None
                    cp_data = engine.calculate_schedule(cp_data)
                    ...
                    st.success(f"✅ Applied {selected_iso} timelines to interconnection milestones!")
                    st.rerun()
                else:
                    st.error("Could not apply timelines")
