...
def generate_portfolio_pdf(site_ids: list, db: Dict, weights: Dict) -> bytes:
    ...
    class PortfolioPDF(FPDF):
        ...
        def header(self):
            if self.page_no() > 1:  # Skip header on cover page
                self.set_font('Helvetica', 'B', 10)
                self.set_text_color(100, 100, 100)
                self.cell(0, 10, 'Portfolio Export | Power Tracker', new_x="LMARGIN", new_y="NEXT", align='L')
                self.ln(2)
                self.line(10, 20, 200, 20)
                self.ln(5)
    ...
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 10, "Data Center Development Sites", new_x="LMARGIN", new_y="NEXT", align='C')
    
    pdf.ln(40)
    
    # Portfolio Summary Box
    pdf.set_fill_color(240, 245, 250)
    pdf.rect(40, pdf.get_y(), 130, 60, style='F')
    ...
    pdf.cell(0, 7, f"Total Sites: {len(sites_data)}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_xy(50, pdf.get_y())
    pdf.cell(0, 7, f"Total Pipeline: {total_mw:,.0f} MW", new_x="LMARGIN", new_y="NEXT")
    pdf.set_xy(50, pdf.get_y())
    
    avg_score = sum(s['scores']['overall_score'] for s in sites_data) / len(sites_data) if sites_data else 0
    pdf.cell(0, 7, f"Average Score: {avg_score:.1f}/100", new_x="LMARGIN", new_y="NEXT")
    pdf.set_xy(50, pdf.get_y())
    pdf.cell(0, 7, f"Breakdown by Stage:", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.cell(0, 10, f"Generated: {datetime.now().strftime('%B %d, %Y')}", new_x="LMARGIN", new_y="NEXT", align='C')
    ...
    for idx, site_data in enumerate(sites_data, 1):
        ...
        state_context = site_data['state_context']
        
        # Site starts on new page
        pdf.add_page()
