...
try:
    from pptx_export_page import show_pptx_export
    PPTX_EXPORT_AVAILABLE = True
except ImportError:
    PPTX_EXPORT_AVAILABLE = False

# Site Profile Builder integration
try:
    from site_profile_page import show_site_profile_builder
    SITE_PROFILE_AVAILABLE = True
except ImportError:
    SITE_PROFILE_AVAILABLE = False
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigation",
        ["📊 Dashboard", "📈 Program Tracker", "📄 Document Context", "📋 Site Profile", "📊 PPTX Export", "💬 AI Chat", "🗺️ PACES Analysis", "📁 VDR Upload", "🏭 Site Database",
         "➕ Add/Edit Site", "🏆 Rankings", "📍 State Analysis", "⚙️ Settings"]
    )
    ...
    if page == "📊 Dashboard": show_dashboard()
        ...
        if SITE_PROFILE_AVAILABLE:
            sites = st.session_state.data.get_all_sites()
            show_site_profile_builder(sites, st.session_state.data)
        else:
            st.error("Site Profile Builder module not found")
    elif page == "📊 PPTX Export":
        if PPTX_EXPORT_AVAILABLE:
            show_pptx_export(st.session_state.data)
        else:
            st.error("PPTX Export module not found")
    elif page == "💬 AI Chat": show_ai_chat()
    elif page == "🗺️ PACES Analysis": show_paces_analysis()
    ...
    elif page == "📍 State Analysis": show_state_analysis()
...
def show_paces_analysis():
    ...
    **Upload PACES screenshots or exports** to automatically extract:
    ...
    if uploaded_files and st.button("🔍 Analyze Images", type="primary"):
        results = []
        ...
        if results:
            ...
            st.success(f"✅ Analyzed {len(results)} image(s)")
    ...
    if 'paces_results' in st.session_state and st.session_state.paces_results:
        ...
        for filename, result in st.session_state.paces_results:
            ...
            if result.overall_suitability:
                color = {'HIGH': '🟢', 'MEDIUM': '🟡', 'LOW': '🔴'}.get(result.overall_suitability.upper(), '⚪')
            ...
            with col3:
                wetland_status = "⚠️ Yes" if result.environmental.wetlands_present else "✅ No"
                st.metric("Wetlands", wetland_status)
            with col4:
                flood_status = "⚠️ Yes" if result.environmental.floodplain_100yr else "✅ No"
        ...
        # Save to site form
        st.subheader("💾 Save to Site Database")
        ...
        if st.button("💾 Save Site", type="primary", key="paces_save"):
